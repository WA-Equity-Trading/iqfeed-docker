...
│  │  4. Write _SILVER_READY, delete _SUCCESS                                 │   │
│  └──────────────────────────────────┬───────────────────────────────────────┘   │
│                                     │ _SILVER_READY                             │
│  ┌──────────────────────────────────▼───────────────────────────────────────┐   │
│  │  s2-ingest-failed                                                        │   │
...
│  │  3. Write _SYMBOLS_DONE, delete _SILVER_READY                            │   │
│  └──────────────────────────────────┬───────────────────────────────────────┘   │
│                                     │ _SYMBOLS_DONE                             │
│  ┌──────────────────────────────────▼───────────────────────────────────────┐   │
│  │  s3-ingest-metadata                                                      │   │
...
│  │  3. Write _METADATA_DONE, delete _SYMBOLS_DONE                           │   │
│  └──────────────────────────────────┬───────────────────────────────────────┘   │
│                                     │ _METADATA_DONE                            │
│  ┌──────────────────────────────────▼───────────────────────────────────────┐   │
│  │  s4-archive-batch                                                        │   │
...
│  │  4. Write _ARCHIVED, delete _METADATA_DONE                               │   │
│  └──────────────────────────────────────────────────────────────────────────┘   │
└──────────────────────────────────────┬───────────────────────────────────────────┘
                                       │
                                       ▼
...
_SUCCESS
  → s1 processes → _SILVER_READY   (s1 deletes _SUCCESS)
      → s2 processes → _SYMBOLS_DONE   (s2 deletes _SILVER_READY)
          → s3 processes → _METADATA_DONE  (s3 deletes _SYMBOLS_DONE)
              → s4 processes → _ARCHIVED    (s4 deletes _METADATA_DONE)
```

Each marker is both the trigger for the next step and deleted by the step that consumed it — preventing double-processing.
...
|---|---|---|---|---|---|
| `s1-ingest-silver` | Eventarc `_SUCCESS` | Batch CSVs | `tick_1sec_001` | `_SUCCESS` | `_SILVER_READY` |
| `s2-ingest-failed` | Eventarc `_SILVER_READY` | `_FAILED.csv` | `failed_symbols` | `_SILVER_READY` | `_SYMBOLS_DONE` |
| `s3-ingest-metadata` | Eventarc `_SYMBOLS_DONE` | BQ queries | `batch_ingest_log` | `_SYMBOLS_DONE` | `_METADATA_DONE` |
| `s4-archive-batch` | Eventarc `_METADATA_DONE` | GCS object list | Archive bucket | `_METADATA_DONE` | `_ARCHIVED` |
...
| `trg-gcs-ingest-tick-silver` | Eventarc Trigger | Fires on `_SUCCESS` |
| `trg-gcs-ingest-failed-symbols` | Eventarc Trigger | Fires on `_SILVER_READY` |
| `trg-gcs-ingest-metadata` | Eventarc Trigger | Fires on `_SYMBOLS_DONE` |
| `trg-gcs-archive-batch` | Eventarc Trigger | Fires on `_METADATA_DONE` |
